     ...
     UNITY_CONST_BUFFER(VxInfo)
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                */
                ...
                // TODO uniform param
                float level1TexSize = 64 ;// round(sqrt(_VoxelParams.z * _VoxelParams.z * _VoxelParams.z));
...
*/
                ...
                if(abs(level1LitInfo.r - 0) < 0.2 || abs(level1LitInfo.r - 1) < 0.2 ){
                     return saturate(level1LitInfo.r   + (1 - level1LitInfo.r) * shadowAlpha);
                }
                else if(abs(level1LitInfo.r - 0.5) < 0.1)
                {
                    ...
                    else if(abs(litOrShadowed - 0) < 0.1 || abs(litOrShadowed - 1) < 1)
                        return saturate(litOrShadowed + (1 - litOrShadowed) * shadowAlpha);
                    else
                        return fixed4(1, 0, 0, 1);
