...
// instanced property arrays
    ...
    #define UNITY_ACCESS_INSTANCED_PROPIDX(arr, var, idx)   arr##Array[idx].var

    #define UNITY_CONST_BUFFER(buf)           UNITY_INSTANCING_CBUFFER_SCOPE_BEGIN(UnityInstancing_##buf) struct { float4 v;} buf[4];  UNITY_INSTANCING_CBUFFER_SCOPE_END
    #define UNITY_CONST_BUFFER_PROP(buf, idx)       buf[idx].v
    ...
    #define UNITY_ACCESS_INSTANCED_PROPIDX(arr, var, idx)           var

    #define UNITY_CONST_BUFFER(buf)
    #define UNITY_CONST_BUFFER_PROP(buf, idx) 0
